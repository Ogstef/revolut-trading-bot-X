...
/**
 * Trades a sample-weighted blend of Reddit + CryptoPanic sentiment for the pair.
 *
 * With {@code require-agreement: true} (default), signals only fire when both
 * sources' signs agree, giving COMBINED the role of a higher-precision filter
 * over the two single-source strategies.
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class CombinedSentimentStrategy implements TradingStrategy {
    ...
    public StrategyType strategyType() {
        return StrategyType.COMBINED_SENTIMENT;
    }
    ...
    public Signal evaluate(BarSeries series, String pair) {
        ...
        SentimentScore score = sentimentService.scoreFor(pair, interval.get(), SentimentSource.COMBINED);
        SentimentConfig.CombinedStrategyThresholds t = config.getStrategies().getCombined();
        return SentimentSignalBuilder.build(
                score, strategyType(),
                t.getBuyThreshold(), t.getSellThreshold(),
                t.isRequireAgreement(),
                price, now);
    }
}
